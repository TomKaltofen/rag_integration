...
def _chained_options(extra: Dict[str, Any] | None = None) -> Options:
    context: Dict[str, Any] = {
        AdjacencyGraphRag.GRAPH_BACKEND: "adjacency",
...
def _run_chained(options: Options) -> List[Dict[str, Any]]:
    feature = Feature(AdjacencyGraphRag.ROOT_FEATURE_NAME, options=options)
    result = mlodaAPI.run_all(
        [feature],
        compute_frameworks={PythonDictFramework},
        plugin_collector=PluginCollector.enabled_feature_groups({AdjacencyGraphRag, TriplesKnowledgeGraph}),
    )
    for partition in result:
        for row in partition:
            if AdjacencyGraphRag.ROOT_FEATURE_NAME in row:
                passages: List[Dict[str, Any]] = row[AdjacencyGraphRag.ROOT_FEATURE_NAME]
                return passages
...
def test_chained_ranking_runs_over_kg_nodes() -> None:
    passages = _run_chained(_chained_options())
